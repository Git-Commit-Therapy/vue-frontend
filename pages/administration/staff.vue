...
import type { Staff } from "~/composable/protobuf/frontend/user";
...
const filteredStaffList = computed(() =>
  staffList.value
    .filter((staff: Staff) =>
      [staff.user!.name, staff.user!.surname].some((s: string) =>
        s.toLowerCase().includes(filterText.value.toLowerCase()),
      ),
    )
    .map((staff: Staff) => staff.user),
);
...
const saveEntity = async () => {
  try {
    const fn = selectedEntity.value.isNew
      ? employeeGRPC.createStaff
      : employeeGRPC.editStaff;
  ...
  <v-container>
    ...
    >
      ...
      <template v-slot:actions>
        <v-btn variant="text" icon="mdi-close" @click="hideError" />
      </template>
    </v-snackbar>
    ...
    <template v-if="!selectedEntity">
      <v-row>
        ...
        <v-col cols="12" md="6" class="text-right">
          <v-btn color="primary" @click="newEntity">{{ t("newStaff") }}</v-btn>
        </v-col>
      </v-row>
    ...
    <template v-else>
      <v-card class="pa-4">
        <v-card-title>{{ t("editStaff") }}</v-card-title>
        <v-card-text>
          <v-row>
            <v-col cols="12" md="6">
              <v-text-field
